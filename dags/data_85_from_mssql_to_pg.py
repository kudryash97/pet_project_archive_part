...
#конфиг DAG
OWNER = "kudryash"
DAG_ID = "data_85_from_mssql_to_pg"
...
SHORT_DESCRIPTION = "Архивные данные 85 из ms sql грузятся в postgres"
...
def transfer_data_85_from_ms_to_pg(**context):
    ...
    logging.info(f"Найдено {len(df)} сигналов 85 подсистемы")
    ...
    df_time = pd.DataFrame({
        ...
        "Mcs": 0, "data": 1, "Pr": 0, "bstate": 1, "bsrc": 2}
    )
    ...
    df_tmp_85_state = result[cols_order]
    df_tmp_85_time = df_time["time_page"]
    df_tmp_85_event = df_tmp_85_state[["time", "Mcs", "num_sign", "data", "Pr", "bstate", "bsrc", "kks_id_signal"]]
    df_tmp_85_state.to_sql(f"tmp_85_{start_date}_state".replace('-', '_'), con=engine_pg, if_exists='replace', index=False)
    df_tmp_85_time.to_sql(f"tmp_85_{start_date}_time".replace('-', '_'), con=engine_pg, if_exists='replace', index=False)
    df_tmp_85_event.to_sql(f"tmp_85_{start_date}_event".replace('-', '_'), con=engine_pg, if_exists='replace', index=False)
...
def column_type_update(**context):
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute(f"""
                    ALTER TABLE tmp_85_{start_date.replace('-', '_')}_event
            ...
            cursor.execute(f"""
                    ALTER TABLE tmp_85_{start_date.replace('-', '_')}_state
            ...
            cursor.execute(f"""
                    ALTER TABLE tmp_85_{start_date.replace('-', '_')}_time
                        ALTER COLUMN time_page TYPE int4;
                """)
...
with DAG(
    ...
    transfer_data_85_from_ms_to_pg = PythonOperator(
        task_id='transfer_data_85_from_ms_to_pg',
        python_callable=transfer_data_85_from_ms_to_pg,
    )
    ...
    start >> transfer_data_85_from_ms_to_pg >> column_type_update >> end